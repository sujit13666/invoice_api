...
class Ion_auth_model extends MY_Model {
    ...
    public function register($identity, $password, $email, $additional_data = array(), $groups = array()) {
        ...
        if ($this->identity_check($identity)) {
            ...
            return FALSE;
        } elseif ($this->username_check($additional_data['username'])) {
        ...
        $key['key'] = md5(uniqid($email, true));
        $key['level'] = 2;
        $key['ignore_limits'] = 0;

        if ($key_saved = $this->keys_model->save($key)) {
            ...
            $saved_user = $this->users_model->save($user_data);
            $saved_user->api_key = $key_saved->key;
            ...
            $user_group['group_id'] = $default_group->id;
            ...
            return ($saved_user) ? $saved_user : FALSE;
        } else {
            $this->set_error('api_not_created');
            return FALSE;
        }
    }
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        $this->users_model->db->join('users_groups_rel', 'users_groups_rel.user_id = users.id');
        $this->users_model->db->join('groups', 'groups.id = users_groups_rel.group_id');
        $user = $this->users_model->get(array(
            $this->identity_column => $identity
                ), $this->identity_column . ', users.email, users.id, users.password, users.active, users.last_login, users.first_name, users.last_name, groups.name as group_name', false, 1);
        ...
        if ($user) {
            ...
            if ($password === TRUE) {
                ...
                $user->is_admin = ($user->group_name == 'admin') ? TRUE : FALSE;
     ...
     *        
     */
    public function rest_login($identity, $password) {
        $this->trigger_events('pre_login');
