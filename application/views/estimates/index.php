		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">
					...
					<a href="<?php print base_url($this->controller.'/edit') ?>" class="btn btn-default pull-right margin-left10"><?php print lang('new_estimate')?></a>
            		...
            		<table class="table table-bordered table-hover table-striped">
                    	<thead>
                        	<tr>
                            	...
                            	<th><?php print lang('estimate') ?> #</th>
                            	<th><?php print lang('client')?></th>
                            	<th><?php print lang('estimate_date')?></th>
                            	<th><?php print lang('expiry_date')?></th>
                            	<th><?php print lang('tax_rate')?>(%)</th>
                            	<th><?php print lang('is_invoiced')?></th>
                     		...
                     		if (isset($result) && isset($result[0]))
                     		{
                     			foreach($result as $key=>$val)
                     			{
                     				print '<tr>';
	                     				...
	                     				print '<td>'.sprintf('%04d',  $val->estimate_number).'</td>';
	                     				print '<td>'.$val->client_name.'</td>';
	                     				print '<td>'.date('m-d-Y',$val->estimate_date).'</td>';
	                     				print '<td>'.(empty($val->due_date) ? '' : date('m-d-Y',$val->due_date)).'</td>';
	                     				print '<td>'.number_format($val->tax_rate,2).'</td>';
	                     				print '<td>'.($val->is_invoiced == 1 ? lang('yes') : lang('no')).'</td>';
	                     				print '<td>'.$val->notes.'</td>';
	                     				print '<td>'.number_format($val->total,2).'</td>';
	                     				print '<td>';
	                     					print '<a href="'.base_url($this->controller.'/delete/'.$val->rid).'" class="pull-right"><i class="fa fa-trash delete-item" data-message="'.lang('delete_estimate_question').'"></i></a>';
