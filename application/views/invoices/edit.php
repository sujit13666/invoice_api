		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">
					<?php print empty($result->id) ? lang('new_invoice') : lang('edit_invoice')?>
		...
		print form_open(base_url($this->controller.'/'.$this->method).'/'.(isset($result->rid) ? $result->rid : ''),
			...
			<div class="row">
				<div class="col-lg-6">
	                ...
	                <div class="form-group">
						<label><?php print lang('invoice_number')?>*</label>
						<input type="text" class="form-control" id="invoice_number" name="invoice_number" value="<?php print set_value('invoice_number', empty($result->invoice_number) ? '' : sprintf('%04d',  $result->invoice_number)) ?>"/>
					</div>
					
					<div class="form-group">
						<label><?php print lang('invoice_date')?>*</label>
						<input type="text" readonly class="form-control" name="invoice_date" id="invoice_date" value="<?php print set_value('invoice_date', (empty($result->invoice_date) ? date('m/d/Y') : date('m/d/Y',$result->invoice_date))) ?>"/>
					</div>
					
					<div class="form-group">
						<label><?php print lang('due_date')?></label>
					...
					<div class="form-group">
						<label class="margin-right10"><?php print lang('payment_received')?></label>
						<input type="checkbox"  name="is_paid" value="1" <?php if (set_value('is_paid', empty($result->is_paid) ? '' : $result->is_paid) != '') {print "checked"; } ?>/>
					</div>
